const swiper = new Swiper(".work__slider", {
	...
	navigation: {
		nextEl: ".work-arrow_right",
		prevEl: ".work-arrow_left",
	},
	breakpoints: {
		"320": {
			slidesPerView: 1,
...
const swiperDown = new Swiper(".contacts__slider", {
	slidesPerView: 1,
	spaceBetween: 10,
	slidesPerGroup: 1,
	autoHeight: true,
	...
	loopFillGroupWithBlank: true,
	navigation: {
		nextEl: ".contacts__right",
		prevEl: ".contacts__left",
	},
});

if (document.querySelector('.header-another')) {
	const headerAnother = document.querySelector('.header-another'),
		linkAnother = headerAnother.querySelectorAll('.menu__link');
	linkAnother.forEach(item => {
		item.addEventListener("click", (e) => {
			const target = e.target,
				dir = target.dataset.goto;
			localStorage.setItem("link", dir);
		});
	});
}
document.addEventListener('DOMContentLoaded', () => {
	if (localStorage.getItem('link')) {
		...
		setTimeout(() => {
			document.querySelector(`[data-goto="${link}"]`).click();
		}, 200);
...
function popup_close(item, bodyUnlock = true) {
	if (unlock) {
		if (!item) {
			...
			let video = item.querySelector('.popup__video iframe');
			if (video) {
...
// Andrikanych Yevhen 2020 + Ilya Vachromeev 2021
...
class DynamicAdapt {
	...
	init() {
		...
		this.arraySort(this.оbjects);// в какуюю то функцию передаётся массив с объектами
	...
	mediaHandler(matchMedia, оbjects) {
		if (matchMedia.matches) {
			оbjects.forEach(item => {
				item.index = this.indexInParent(item.parent, item.element);
				this.moveTo(item.place, item.element, item.destination);
			});
		} else {
			оbjects.forEach(item => {
				if (item.element.classList.contains(this.daClassname)) {
					this.moveBack(item.parent, item.element, item.index);
				}
			});
		}
	}
